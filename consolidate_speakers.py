#!/usr/bin/env python3
"""
Speaker Data Consolidation Script
Transforms data from 9 MongoDB databases into a unified structure
"""
...
TARGET_DB = "expert_finder_unified"
...
class SpeakerConsolidator:
    ...
    def run(self):
        ...
        logger.info("Starting speaker data consolidation...")
    ...
    def calculate_profile_score(self, profile: Dict[str, Any]) -> int:
        ...
        # Topics (15 points)
        if len(profile['expertise'].get('primary_topics', [])) > 0:
            score += 10
        if len(profile['expertise'].get('all_topics', [])) > 3:
            score += 5
    ...
    def merge_profiles(self, existing: Dict[str, Any], new_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Professional info
        if not existing['professional_info'].get('title') and new_data['professional_info'].get('title'):
            existing['professional_info']['title'] = new_data['professional_info']['title']
        if not existing['professional_info'].get('company') and new_data['professional_info'].get('company'):
            existing['professional_info']['company'] = new_data['professional_info']['company']
            
        # Biography - keep longest
        if new_data['biography'].get('full'):
            existing_bio = existing['biography'].get('full', '')
            if len(new_data['biography']['full']) > len(existing_bio):
                existing['biography']['full'] = new_data['biography']['full']
                
        # Location - prefer most complete
        if not existing['location'].get('city') and new_data['location'].get('city'):
            existing['location'] = new_data['location']
            
        # Topics - combine
        existing_topics = set(existing['expertise'].get('all_topics', []))
        new_topics = set(new_data['expertise'].get('all_topics', []))
        all_topics = list(existing_topics.union(new_topics))
        primary_topics, _ = self.normalize_topics(all_topics)
        existing['expertise']['primary_topics'] = primary_topics
        existing['expertise']['all_topics'] = all_topics
        ...
        # Contact - prefer direct contact
        if new_data['contact'].get('email') and not existing['contact'].get('email'):
            existing['contact']['email'] = new_data['contact']['email']
    ...
    def process_a_speakers(self):
        ...
        for doc in collection.find():
            try:
                ...
                # Topics
                primary_topics, all_topics = self.normalize_topics(doc.get('topics', []))
                profile['expertise']['primary_topics'] = primary_topics
                profile['expertise']['all_topics'] = all_topics
    ...
    def process_allamericanspeakers(self):
        ...
        for doc in collection.find():
            try:
                ...
                # Topics
                primary_topics, all_topics = self.normalize_topics(doc.get('speaking_topics', []))
                profile['expertise']['primary_topics'] = primary_topics
                profile['expertise']['all_topics'] = all_topics
    ...
    def process_eventraptor(self):
        ...
        for doc in collection.find():
            try:
                ...
                primary_topics, all_topics = self.normalize_topics(doc.get('business_areas', []))
                profile['expertise']['primary_topics'] = primary_topics
                profile['expertise']['all_topics'] = all_topics
    ...
    def process_freespeakerbureau(self):
        ...
        for doc in collection.find():
            try:
                ...
                profile['expertise']['primary_topics'] = primary_topics
                profile['expertise']['all_topics'] = all_topics
                profile['expertise']['categories'] = doc.get('areas_of_expertise', [])
    ...
    def create_indexes(self):
        ...
        collection.create_index("expertise.primary_topics")
...
def main():
    ...
    consolidator = SpeakerConsolidator()
    consolidator.run()
